...
Plany zbudowane na błędnym sformułowaniu problemu są doskonałymi rozwiązaniami dla niewłaściwego pytania. Ta umiejętność służy jednemu celowi: oddzieleniu **obserwacji** od **podanej przyczyny** — oraz **problemu** od **proponowanego rozwiązania** — zanim rozpocznie się jakiekolwiek planowanie.

Kształt, którym zajmuje się ta umiejętność, jest ogólny: użytkownik opisuje coś (obserwację, postrzegany problem, zakres, którym chce się zająć) i w tym samym zdaniu proponuje odpowiedź (przyczynę, podejście, strukturę planu). Te dwie rzeczy są traktowane jako jeden fakt. Doskonały /10x-plan dostarcza wtedy doskonałe rozwiązanie — a rzeczywisty problem pozostaje, ponieważ ramy były błędne; plan był prawidłowy; użytkownik stracił dzień.
...
W razie wątpliwości, ta umiejętność zadaje krótką serię pytań i szybko kończy, jeśli ramy okażą się solidne. Koszt uruchomienia jej na jasnym żądaniu: ~2–3 pytania. Koszt pominięcia jej na błędnie sformułowanym zadaniu: błędny plan i stracony dzień.
...
1. **Jeśli podano ścieżkę pliku lub change-id** (np. `/10x-frame @context/changes/foo/research.md` lub `/10x-frame foo`), rozwiąż ją: `<change-id>` rozwiązuje się do `context/changes/<change-id>/research.md` (przeczytaj, jeśli istnieje). Przeczytaj plik W CAŁOŚCI i przejdź do Kroku 1.
...
Przeczytaj `context/foundation/lessons.md`, jeśli istnieje, i użyj wcześniejszych lekcji dotyczących kształtu ram (powtarzające się pułapki ramowania i akceptowane zasady) jako priorytetów podczas konstruowania mapy wymiarów w Kroku 2 — są one kontekstem nośnym, a nie opcjonalnym czytaniem.
...
Ten krok zawsze jest wykonywany. Przed zbudowaniem mapy wymiarów (Krok 2) lub wysłaniem równoległych podagentów (Krok 3), wstrzymaj się na jedną rundę pytań wyjaśniających przy każdym wywołaniu. Celem jest usunięcie niejasności dotyczących *obserwacji i zakresu* — "który z tych elementów jest głównym problemem?", "czy to jedna obserwacja, czy kilka?", "czy obserwowalny jest pojedynczym objawem, czy klasą objawów?" — tak, aby mapa wymiarów była budowana na podstawie skoncentrowanej obserwacji, a nie wielopunktowej listy zadań.

Zapytaj użytkownika:
- question: "Który z tych elementów jest głównym problemem? Czy to jedna obserwacja, czy kilka? Czy obserwowalny jest pojedynczym objawem, czy klasą objawów?"
  options:
  - label: "To jest główny problem"
    description: "Skoncentruj się na tej konkretnej obserwacji."
  - label: "To jest kilka obserwacji"
    description: "Traktuj je jako odrębne problemy."
  - label: "To jest pojedynczy objaw"
    description: "To jeden konkretny problem."
  - label: "To jest klasa objawów"
    description: "Reprezentuje szerszą kategorię problemów."
  - label: "Nie jestem pewien / jeszcze ich nie rozdzieliłem"
    description: "Potrzebuję pomocy w rozróżnieniu między nimi."

Te pytania są ograniczone przez poniższą zasadę #4 ("Pytania zawężające ≠ pytania o rozwiązanie"). Pytania przed wysyłką opisują obserwacje lub pozycje zakresu, nigdy przyczyny ani poprawki. Jeśli zauważysz, że tworzysz opcję, która proponuje poprawkę lub podejście, wkroczyłeś na terytorium /10x-plan — zatrzymaj się i przepisz ją jako obserwację.

Zapisz odpowiedzi w rejestrze ramowania obok zapisu z Kroku 1; Frame Brief z Kroku 6 zachowuje oba jako oddzielne punkty pod "Initial Framing (preserved)" (nowa linia `Pre-dispatch narrowing`). Oryginalna obserwacja, podana przyczyna i proponowany kierunek pozostają dosłowne z Kroku 1; zawężenie z Kroku 1.5 nakłada się na nie, nie zastępując ich.
...
- **Najpierw przeczytaj.** Otwórz pliki, o których wspomniał użytkownik. Otwórz sąsiednie. Śledź ścieżkę od podanej przyczyny do zaobserwowanego efektu — *niezależnie od tego, czy ta ścieżka to przepływ danych w czasie wykonania, łańcuch decyzji projektowych, czy sekwencja założeń*. Wymiary wynikają z tego, co faktycznie istnieje: etapy wejścia, transformacji, stanu, efektów ubocznych; lub osie przestrzeni projektowej; lub warstwy decyzji o zakresie. Nie wymieniaj wymiarów, dla których nie widziałeś dowodów.
- **Użyj podagentów, gdy powierzchnia jest duża lub nieznana.** Utwórz jednego lub dwóch podagentów typu Explore z promptami takimi jak: "Śledź ścieżkę od <podanej przyczyny> do <zaobserwowanego efektu>. Wymień każdy odrębny etap lub oś, przez którą przechodzi łańcuch, z odniesieniami file:line lub document:section." Mapa to to, co zwracają — a nie to, co zgadłeś przed przeczytaniem.
- **Traktuj każdy wymiar jako możliwe źródło.** Użyteczny wymiar to taki, w którym, gdyby ramy pękły w tym miejscu, zobaczyłbyś mniej więcej tę obserwację. Wymiary, które nie mogłyby wiarygodnie wywołać obserwacji, nie należą do mapy.
...
Zarejestruj jedno zadanie dla każdego wiarygodnego wymiaru. Następnie utwórz równoległe podagenty — zazwyczaj 2–4, maksymalnie 5 — używając narzędzia Task, **wszystkie w jednej wiadomości** dla współbieżności.
...
- Ramowanie oczekiwanych dowodów: "Co byśmy zobaczyli, gdyby TO był wymiar, w którym ramy pękają? Szukaj tego. Zgłoś, czy jest obecne, częściowe, czy nieobecne, z odniesieniami file:line lub document:section."
...
Zadaj użytkownikowi pytania. **Pytania i opcje tutaj są zasadniczo różne od tych w /10x-plan**: w /10x-plan opcje to *wybory rozwiązania*; tutaj opcje to *rozróżnienia hipotez*. Odpowiedź użytkownika zawęża przestrzeń hipotez.
...
- Celuj w 2–5 pytań łącznie — wystarczająco, aby triangulować, ale nie na tyle, aby się przeciągać.
- ZAWSZE dołącz opcję "Nie jestem pewien / nie sprawdziłem". Pewność użytkownika sama w sobie jest sygnałem; fałszywa pewność jest wrogiem.

Pytanie zawężające, które nie zmienia rankingu hipotez, jest marnotrawstwem. **Zaprojektuj każde pytanie tak, aby było decydujące.** Jedno dobrze ukierunkowane pytanie, na które udzielono szczerej odpowiedzi, często samo rozwiązuje całe pytanie o przeformułowanie.
...
Przed sfinalizowaniem przeformułowania, przetestuj je z innej perspektywy niż badanie, które je wygenerowało. Celem jest ujawnienie dowodów, których badanie hipotezy nie zauważyło, a nie potwierdzenie tego, w co już wierzysz.

Wybierz te, które są przydatne w danym przypadku:

- **Niezależne wyszukiwanie.** Utwórz nowego podagenta Explore z promptem, który NIE wymienia wiodącej hipotezy. Opisz tylko obserwację i zapytaj: "Co w tym systemie lub przestrzeni projektowej jest najbardziej prawdopodobną przyczyną? Szukaj bez uprzedzeń." Jeśli agent niezależnie dojdzie do tej samej hipotezy, pewność wzrasta. Jeśli ujawni coś innego, to jest to sygnał, który warto dokładnie przeczytać.
...
- **Sprawdź odwrotność.** Jakie inne dowody przewidywałaby wiodąca hipoteza — których jeszcze nie sprawdziłeś? Zweryfikuj to. Co NIE powinno być widoczne, jeśli hipoteza jest prawdziwa? Potwierdź jej brak.
...
Rozwiąż folder zmian przed pisaniem:

- Jeśli wywołano jako `/10x-frame <change-id>` i `context/changes/<change-id>/` istnieje, zapisz do niego.
- W przeciwnym razie utwórz kebab-case `<change-id>` z obserwacji i utwórz folder + `change.md` (odzwierciedlając semantykę `/10x-new`) przed zapisaniem.
...
Wydrukuj jednookranowe podsumowanie, a następnie zaoferuj przekazanie:
...
2. **Obserwacja i podana przyczyna pozostają oddzielne.** Na każdym etapie. Frame Brief zachowuje oryginalne ramy dosłownie — nawet po przeformułowaniu — ponieważ przyszli czytelnicy (i /10x-plan-review) muszą widzieć, co było założone, a co odkryte.
...
7. **Ogranicz czas badania.** Frame powinien zazwyczaj zakończyć się w 2–4 rundach podagentów i 2–5 pytaniach. Jeśli przeciąga się to poza ten czas, przypadek prawdopodobnie wymaga reprodukcji lub zbierania dowodów przed dalszą analizą — zalec to i zatrzymaj się.
...
- Rozróżniaj "dowody znalezione w tym projekcie" (weryfikowalne, z file:line lub document:section) od "mam przeczucie z poprzednich systemów, które widziałem" (wcześniejsze, niezweryfikowane). Wcześniejsze dane są przydatne do formułowania hipotez; tylko zweryfikowane dowody należą do Frame Brief.
...
- Frame Brief to jedyny artefakt. Zachowaj go krótkim, łatwym do zeskanowania i możliwym do działania dla /10x-plan.
