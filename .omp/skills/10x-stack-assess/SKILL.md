...
# Ocena Stosu Technologicznego: Oceń Istniejący Stos pod kątem Przyjazności dla Agentów AI

Ta umiejętność jest odpowiednikiem `/10x-tech-stack-selector` dla projektów brownfield. Podczas gdy tech-stack-selector pomaga użytkownikom greenfield **wybrać** stos technologiczny, stack-assess pomaga użytkownikom brownfield **ocenić** ich obecny stos. Wykorzystuje te same cztery bramki jakości przyjazne dla agentów (`references/agent-friendly-criteria.md`), ale stosuje je jako soczewkę oceny, a nie filtr wyboru.

Umiejętność ta wpisuje się w łańcuch brownfield: `/10x-shape → /10x-prd → /10x-stack-assess → /10x-health-check`. Jej jedynym zadaniem jest ocena istniejącego stosu pod kątem bramek jakości i stworzenie ustrukturyzowanej oceny z konkretnymi strategiami kompensacji.

Kluczową wartością dla projektów brownfield jest **ścieżka kompensacji** — gdy bramka zawodzi, umiejętność nie zaleca wymiany stosu. Dokumentuje, co należy dodać do plików instrukcji (CLAUDE.md / AGENTS.md), aby agent mógł skutecznie działać pomimo luki.

## Kiedy używać, kiedy pominąć

**Użyj, gdy**: użytkownik ma istniejący projekt i chce ocenić, jak dobrze jego stos technologiczny wspiera przepływy pracy agentów AI. Katalog projektu powinien zawierać rozpoznawalne znaczniki projektu (`package.json`, `Cargo.toml`, `pyproject.toml`, `go.mod`, `Gemfile`, `composer.json`, `*.csproj`, `pubspec.yaml`). Opcjonalnie, `context/foundation/prd.md` (brownfield) istnieje — jeśli jest obecny, umiejętność wykorzystuje go do kontekstualizacji oceny (np. które komponenty są w zakresie zmian).
...
- `/10x-tech-stack-selector` — równoległy dla greenfield. Te same bramki jakości, inne zadanie (wybór vs ocena).
- `/10x-health-check` — konsument downstream. Odczytuje `context/foundation/stack-assessment.md`, aby skupić kontrole stanu na zidentyfikowanych lukach.
...
Jeśli znaczniki zostaną znalezione, przejdź do Kroku 1.
...
- **Zaliczenie**: język domyślnie używa jawnych typów (TypeScript, Rust, Go, Java, Kotlin, C#, Dart) LUB projekt ma skonfigurowane sprawdzanie typów (Python + mypy/pyright w zależnościach/konfiguracji, Ruby + Sorbet/RBS, PHP + PHPStan/Psalm).
- **Niezaliczenie**: JavaScript bez TypeScript, Python bez skonfigurowanego sprawdzania typów, Ruby bez Sorbet/RBS, PHP bez analizy statycznej.
...
- **Zaliczenie**: framework narzuca silne opinie dotyczące układu folderów, routingu, konfiguracji (Next.js App Router, Rails, Django, Spring Boot, Astro, Angular, Laravel, .NET).
- **Niezaliczenie**: framework jest minimalistyczny/nieoparty na opiniach, a projekt nie ma udokumentowanych konwencji (Express, Koa, Flask bez blueprintów, Sinatra, czysty Vite + React).
- **Częściowe zaliczenie**: minimalistyczny framework, ALE projekt ma udokumentowane konwencje w plikach instrukcji (CLAUDE.md, AGENTS.MD) lub widoczny dokument konwencji. Ocena jako zaliczenie z uwagą.
...
- **Zaliczenie**: framework jest głównym wyborem w swoim ekosystemie językowym (React, Next.js, Vue, Angular w JS; Django, FastAPI, Flask w Pythonie; Rails w Ruby; Spring w Javie; Laravel w PHP; .NET w C#; Flutter w Dart).
- **Niezaliczenie**: niszowy lub bardzo nowy framework z ograniczonymi danymi treningowymi w swojej rodzinie języków.
- **Dowód**: nazwij framework i jego pozycję w ekosystemie językowym.

#### Bramka 4: Dobrze udokumentowany

- **Zaliczenie**: framework ma aktualną, wersjonowaną oficjalną dokumentację.
- **Niezaliczenie**: dokumentacja jest rozproszona, nieaktualna lub wiki utrzymywane przez społeczność jest niezsynchronizowane.
...
**Oparty na konwencjach: niezaliczenie** →
...
**Popularny w danych treningowych: niezaliczenie** →
...
**Dobrze udokumentowany: niezaliczenie** →
...
- **ready**: wszystkie bramki zaliczone dla wszystkich komponentów. Stos jest przyjazny dla agentów od razu.
- **ready-with-compensation**: niektóre bramki niezaliczone, ale wszystkie niezaliczenia mają jasne strategie kompensacji. Stos działa z udokumentowanymi konwencjami.
...
Werdykt ma charakter informacyjny, nie blokujący. Nawet `significant-friction` nie oznacza "zmień stos" — oznacza "zarezerwuj więcej czasu na tworzenie plików instrukcji i spodziewaj się więcej cykli korekcji agenta".

### Krok 5 — Napisz ocenę

Sprawdź kolizję, próbując odczytać `context/foundation/stack-assessment.md`.
...
### Zalecane Dodatki do Pliku Instrukcji
...
7. **Tylko uniwersalny język.** Brak prywatnych ścieżek do skarbca lub brandingu specyficznego dla organizacji w dostarczanej treści.
